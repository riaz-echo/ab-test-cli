(() => {
    ...
    function rebindOffcanvasNativeControls(offcanvas) {
        try {
            ...
            const shippingToggle = offcanvas.querySelector(".js-toggle-shipping-selection");
            if (shippingToggle) {
                shippingToggle.addEventListener("click", () => {
                    const summary = shippingToggle.closest(".offcanvas-summary");
                    const preference = summary && summary.querySelector(".offcanvas-shipping-preference");
                    if (preference) preference.classList.toggle("offcanvas-shipping-preference--hidden");
                });
            }
        } catch (error) {}
    }
