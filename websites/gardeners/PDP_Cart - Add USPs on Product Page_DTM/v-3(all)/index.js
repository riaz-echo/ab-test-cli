(() => {
    ...
    const BODY_CLASS = "ab--all-usp";
    const USP_CLASS = "ab--usp-bar";

    const PLACEMENTS = [
        {
            name: "pdp",
            modifier: "ab--usp-bar--pdp",
            targets: ["form.product-form"],
            position: "afterend",
            watches: ["form.product-form"],
        },
        {
            name: "side-cart",
            modifier: "ab--usp-bar--side-cart",
            targets: [".quick-cart__items", ".quick-cart_items"],
            position: "afterbegin",
    ...
    PLACEMENTS.forEach((placement) => {
        placement.targets = [].concat(placement.targets);
        placement.watches = [].concat(placement.watches);
    });
...
`,
    ...
    function reportPlacements() {
        setTimeout(() => {
            const summary = PLACEMENTS.map((placement) => `${placement.name}: ${document.querySelector(`.${placement.modifier}`) ? "yes" : "no"}`).join(", ");

            logInfo(`placed - ${summary}`);
        }, 10000);
    }
